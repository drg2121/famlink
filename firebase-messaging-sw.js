...
/* ANTI-DUBLURĂ prin memorie persistentă: dacă serverul trimite aceeași notificare de
   ...
   la ACELAȘI service worker. Reținem semnătura (titlu+corp) în Cache API și, dacă una
   identică a fost afișată în ultimele 90 s, NU o mai afișăm a doua oară. */
function famlinkDedupe(d) {
  try {
    var sig = 'flnd:' + encodeURIComponent(String(d.title || '') + '|' + String(d.body || ''));
    return caches.open('famlink-notif-dedupe').then(function (c) {
      return c.match(sig).then(function (m) {
        var now = Date.now();
        if (m) {
          return m.text().then(function (ts) {
            if (now - Number(ts) < 90000) return true;             // deja afișată recent → duplicat
            return c.put(sig, new Response(String(now))).then(function () { return false; });
          });
        }
        return c.put(sig, new Response(String(now))).then(function () { return false; });
      });
