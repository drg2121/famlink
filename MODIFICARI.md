...
## v4.1.7 — pașii ca în aplicația Sănătate, fără să depindă de ceas (2026-09-24)

- **Fără filtru Source.** Filtrul „Source contains Watch” a întors pașii goi (numele ceasului nu se potrivea), iar utilizatorul nu vrea ca pașii să depindă de ceas. Voia cifra din aplicația Sănătate, cu telefonul și ceasul numărate o singură dată.
- **Scurtătura trimite mostrele de pași:** sursa (`ps`), valoarea (`pv`) și ora de început (`pt`) a fiecărei mostre, separate prin `|`, obținute cu Get Details of Health Samples + Combine Text. `steps` rămâne suma brută, ca rezervă.
